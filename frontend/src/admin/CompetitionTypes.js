...
export default function CompetitionTypes() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await api.delete(`/competition-dictionaries/types/${id}`);
      if (res.success) {
        fetchData();
      } else {
    ...
    <AdminShell>
      <div className="ap-card" style={{ maxWidth: '1000px', margin: '20px auto', padding: '30px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
          <h1 style={{ fontSize: '24px', fontWeight: 'bold' }}>Жарыс түрлері</h1>
          <Link to="/admin/olympiads/types/new" className="ap-btn ap-btn-primary">
                ...
                data.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <button onClick={() => navigate(`/admin/olympiads/types/${item.id}/edit`)} style={{ color: '#3b82f6', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', padding: '5px', marginRight: '5px' }}>
